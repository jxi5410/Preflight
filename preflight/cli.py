"""Preflight CLI — main entry point.

Usage:
    preflight run <url> [options]
    preflight schedule <url> --cron "0 2 * * *" [options]
    preflight compare <baseline_dir> <current_dir>
    preflight export-issues --repo <repo> --run <run_dir>
"""
...
from preflight.core.schemas import Credentials, RunConfig
...
@click.pass_context
def main(ctx):
    """Preflight — External-experience AI QA system.
...
def _interactive_run():
    ...
    console.print("\n[bold]Welcome to Preflight[/bold] — your team of AI QA companions\n")
    ...
    if mode == "quick":
        console.print(f"\n[bold]Preflight Quick Check[/bold] — [cyan]{url}[/cyan]\n")
        setup_logging(verbose=True)
        from preflight.core.quick_check import quick_check as run_quick_check
    ...
    console.print(f"[bold]Preflight[/bold] evaluating: [cyan]{url}[/cyan]")
    if repo:
...
def run(
    ...
    console.print(f"\n[bold]Preflight[/bold] evaluating: [cyan]{url}[/cyan]")
    if repo:
        console.print(f"  Repository: [cyan]{repo}[/cyan]")
    console.print()
...
def check(url: str, focus: str | None, tier: str, json_out: bool, verbose: bool):
    ...
    console.print(f"\n[bold]Preflight Quick Check[/bold] — {result.url}")
...
def schedule(
    ...
    from preflight.scheduling.scheduler import RunScheduler
...
def compare(baseline_dir: str, current_dir: str, output: str | None, verbose: bool):
    """Compare two Preflight runs to detect regressions."""
    setup_logging(verbose)
    ...
    console.print(f"\n[bold]Preflight Run Comparison[/bold]")
...
@click.option("--run", "run_dir", required=True, help="Path to Preflight run artifacts")
...
def export_issues(repo: str, run_dir: str, min_severity: str, dry_run: bool, verbose: bool):
    """Export Preflight findings as GitHub issues."""
    setup_logging(verbose)

    from preflight.reporting.comparison import load_run_result
    from preflight.reporting.github_export import export_issues_via_gh, export_summary
    ...
    console.print(f"\n[bold]Exporting Preflight issues to GitHub[/bold]")
...
def handoff(run_dir: str, fmt: str, repo: str | None, github_token_env: str, output: str | None, verbose: bool):
    ...
    Regenerate handoff from an existing run in a specific format:
        ...
        preflight handoff ./artifacts --format codex
    """
    setup_logging(verbose)
