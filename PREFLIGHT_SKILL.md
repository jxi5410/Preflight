# Preflight — Claude Code Integration Guide

## What is Preflight?

Preflight is an external-experience AI QA system that evaluates web products like real users would. It generates realistic user personas, drives browser-based evaluations, applies specialist lenses (design, trust, auth, responsiveness), and produces evidence-backed reports with developer-ready handoffs.

## MCP Server Setup

Add Preflight to your Claude Code MCP configuration:
...
→ Uses preflight_quick_check with focus="login flow"
```

### Full evaluation with repo context
```
"Run a full Preflight evaluation on https://myapp.com with repo https://github.com/org/myapp"
→ Uses preflight_evaluate with repo_url for deeper analysis
```

### Review findings after evaluation
```
"Show me the Preflight report"
→ Uses preflight_get_report with format="markdown"
```

### Regression check
```
"Compare the current Preflight run against last week's baseline in ./baseline"
→ Uses preflight_compare with baseline_dir and current_dir
```

### CI gate check
```
"Evaluate https://staging.myapp.com and fail if there are any high-severity issues"
→ Uses preflight_evaluate with fail_on="high"
```

## CLI Commands

Preflight also works as a standalone CLI:

```bash
# Quick check
preflight check https://your-product.com
preflight check https://your-product.com --focus "checkout flow" --json-output

# Full evaluation
preflight run https://your-product.com --repo https://github.com/user/repo
preflight run https://your-product.com --tier premium --fail-on high
